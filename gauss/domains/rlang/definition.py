...
from gauss.domains.rlang.graphs import ELabel, NLabel, DataFrameGraph, GraphRLang
from gauss.domains.rlang.strategies import RandomizedGraphStrategy, DfsGraphStrategy, \
...
class RLangSynthesisDomain(SynthesisDomain):
    ...
    def prepare_solution(self,
                         ...
                         int_to_obj: Dict[int, Any]) -> RLangSolution:
        program_lines = []
    ...
    def rank_sequences(self, sequences: List[List[str]]) -> List[List[str]]:
        return sorted(sequences,
                      key=lambda x: -{
                          ('rf.select',): 3,
                          ('rf.group_by',): 2,
                          ('rf.gather', 'rf.group_by'): 1,
                          ('rf.mutate', 'rf.select'): 2,
                      }.get(tuple(x), 0))


@attr.s(cmp=False, repr=False)
class RLangSynthesisUI(SynthesisUI):
    AGGREGATIONS = {
...
def _generate_computation_node(interaction: Dict, graph: GraphRLang, g_inputs: Dict[str, DataFrameGraph]) -> Node:
